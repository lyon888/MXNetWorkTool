...
#ifdef DEBUG
//#define MXLog(s, ... ) printf(@"[%@ in line %d] ---->>>>>>%@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [[NSString stringWithFormat:(s), ##__VA_ARGS__]UTF8String] )
...
typedef NS_ENUM(NSUInteger, MXNetworkStatus) {
    kMXNetworkStatusUnknown          = 111,//未知网络
    kMXNetworkStatusNotReachable     = 0,//网络无连接
    kMXNetworkStatusReachableViaWWAN = 1,//2，3，4G网络
    kMXNetworkStatusReachableViaWiFi = 2,//WIFI网络
};

/**
 @author 刘智援, 16-10-31 11:17:50
...
typedef void(^MXResponseSuccess)(id response);
typedef void(^MXResponseFail)(NSError *error);
